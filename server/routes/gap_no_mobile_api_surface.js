/**
 * Gap Feature: gap-no-mobile-api-surface
 * No mobile API surface.
 *
 * POST /api/gap-no-mobile-api-surface
 * Auth required. Generated as part of Gaps backend scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_GAP_NO_MOBILE_API_SURFACE_KEY
...
const FEATURE_KEY = process.env.FEATURE_GAP_NO_MOBILE_API_SURFACE_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: No mobile API surface..\nGoal: No mobile API surface.\nRespond with clear, actionable analysis. Prefer JSON when structured output is requested.`;
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDebtCollectionOptimizer - gap-no-mobile-api-surface',
    },
    body: JSON.stringify({
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_GAP_NO_MOBILE_API_SURFACE_KEY');
    const ai = await callLLM({ feature: 'gap-no-mobile-api-surface', goal: 'No mobile API surface.', input: payload });
    try {
      ...
      if (pool && pool.query) {
        await pool.query(
          ...
          ['gap-no-mobile-api-surface', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[gap-no-mobile-api-surface] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'gap-no-mobile-api-surface',
      endpoint: '/api/gap-no-mobile-api-surface',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[gap-no-mobile-api-surface] error:', err.message);
