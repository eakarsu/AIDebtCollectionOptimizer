/**
 * Custom Feature: fraud-detect
 * Fraud detection across debtor patterns
 *
 * POST /api/ai/fraud-detect
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_FRAUD_DETECT_KEY
...
const FEATURE_KEY = process.env.FEATURE_FRAUD_DETECT_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Fraud detection across debtor patterns.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDebtCollectionOptimizer - fraud-detect',
    },
    body: JSON.stringify({
...
router.post('/fraud-detect', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_FRAUD_DETECT_KEY');
    const ai = await callLLM({ feature: 'fraud-detect', goal: 'Fraud detection across debtor patterns', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['fraud-detect', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[fraud-detect] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'fraud-detect',
      endpoint: '/api/ai/fraud-detect',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/fraud-detect/health', (req, res) => {
  res.json({
    feature: 'fraud-detect',
    endpoint: '/api/ai/fraud-detect',
